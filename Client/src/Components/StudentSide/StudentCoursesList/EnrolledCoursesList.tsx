...
function EnrolledCoursesList() {
  ...
  const parseData=JSON.parse(student)
    ...
    useEffect(() => {
        axios.get(`${Base_Url}/student/getenrolledcourses/${parseData?._id}`)
          .then((response) => {
      ...
      <Row>
        {allCourseList.length !== 0 ? (
          allCourseList.map((course) => (
            <Col key={course._id}> 
              <Card style={{ width: '16vw', height: '25rem' }} className='mt-4 justify-content-center align-items-center'>
                <Card.Img style={{ height: '15rem' }} variant="top" src={`${Image_Url}/${course?.photo}`} /> 
          ...
          <h2 style={{ color: 'red',fontStyle:'italic'  }}>Sorry No enrolled courses available ! </h2>
