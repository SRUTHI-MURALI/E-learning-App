...
interface StudentPdfLessonsProps {
  courseId: Lesson[] | null;
}

function StudentPdfLessons({ courseId }: StudentPdfLessonsProps) {
  return (
    <div>
      <Container>
        {courseId?.length !== 0 && (
          ...
          <Row className="m-3" key={courses._id}>
            <Accordion>
              <Accordion.Item eventKey={courses._id}>
                ...
                <Accordion.Body>Description : {courses?.description}</Accordion.Body>
                <Accordion.Body>
                  PDF : {courses?.pdf && (
                    <a
                      href={courses?.pdf}
